...
public class UIRenderer {
    ...
    public void renderMenuBackgroundThin(int buttonPosX,int buttonMargin,int widthTiles,int heightTiles){
        for (int x = buttonMargin; x<widthTiles-buttonMargin; x++) {
            //this.menuRenderer.renderTex(17, heightTiles, widthTiles, x, 5);
            if (x == buttonMargin){
                this.renderTex(boxTextures[0] + textureOffset, heightTiles, widthTiles, x, buttonPosX + 1);
                this.renderTex(boxTextures[6] + textureOffset, heightTiles, widthTiles, x, buttonPosX);
            }else if(x==widthTiles-buttonMargin-1){
                this.renderTex(boxTextures[2] + textureOffset, heightTiles, widthTiles, x, buttonPosX + 1);
                this.renderTex(boxTextures[8] + textureOffset, heightTiles, widthTiles, x, buttonPosX);
            }else {
                this.renderTex(boxTextures[1] + textureOffset, heightTiles, widthTiles, x, buttonPosX + 1);
                this.renderTex(boxTextures[7] + textureOffset, heightTiles, widthTiles, x, buttonPosX);
            }
        }
    }
