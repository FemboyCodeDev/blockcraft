...
import com.meowjang.rubydung.networking.Client;
import com.meowjang.rubydung.networking.CreatePacket;
...
public class RubyDung implements Runnable {
    ...
    private int game_mode = 0; // 0: Main menu | 1: Save select | 2: Create Save | 10: Settings| 11: Graphics Settings| 100: In game | 101: Paused game | 200: Server Select
    private boolean dev_command_pause = true;

    private boolean vsync = false;

    float fps;
    final String version = "0.0.1e";
   ...
   public void connect_to_server (String address, int port) throws IOException {
       this.client.initialize(address, port);
    ...
    public void run() {
        ...
        try {
                ...
                while (System.currentTimeMillis() >= lastTime + 1000L) {
                    ...
                    if (game_mode == 100) {
                        ...
                        if (this.is_multiplayer){
                            int[] serverData = {(int)player.x,(int)player.y,(int)player.z};
                            client.sendBytes(CreatePacket.createPacket(16, 1, serverData));
                        }
            ...
            JOptionPane.showMessageDialog((Component) null, var10.toString(), "Runtime Error", 0);
        ...
        if (Keyboard.isKeyDown(207)) {
            ...
            for (int x = 0; x < level.width; ++x) {
                for (int y = 0; y < level.height; ++y) {
                    level.setTile((int) x, (int) player.y - 2, (int) y, 0);
                }
            }
    ...
    public void render_mainMenu(float a) throws IOException {
        ...
        String[] options = {"play_alone", "play_with_friends", "change_settings", "goodbye"};
        String[] option_text = {"SinglePlayer", "MultiPlayer", "Settings", "QUIT"};
        int[] options_offsets = {0, 0, 0, 0};

        buttonPos = heightTiles - 6;
        ...
        for (int i = 0; i < options.length; i++) {
            ...
            float y1 = tileSizeY * (buttonPos + 2 - (i * 3));
            float x2 = x1 - tileSizeX;
            float y2 = y1 - (tileSizeY * 3);
            ...
            if (mouseY < y1 && mouseY > y2) {
                ...
                if (Mouse.isButtonDown(0)) {
                    if (!this.menu_button_pressed) {
                        if (options[i] == "play_alone") {
                            this.game_mode = 1;
                            this.menu_button_pressed = true;
                            //return;
                        }
                        if (options[i] == "play_with_friends") {
                            this.game_mode = 100;
                            this.connect_to_server("127.0.0.1",5000);
                            this.menu_button_pressed = true;
                            this.level.load("levels\\new game.dat");
                            //return;
                        }
                        if (options[i] == "change_settings") {
                            this.game_mode = 10;
                            this.menu_button_pressed = true;
                            //return;
                        }
                        if (options[i] == "goodbye") {
                            this.game_mode = 10;
                            this.menu_button_pressed = true;
                            //return;
                        }
            ...
            this.menuRenderer.renderMenuBackground(buttonPos - (i * 3), buttonMargin, widthTiles, heightTiles);
            this.fontRenderer.renderText(buttonMargin + 0.5f, buttonPos + 0.5f - (i * 3), option_text[i], widthTiles, heightTiles);
            this.menuRenderer.textureOffset = 0;
        }
    ...
    public void render_mainMenu_old(float a) {
       ...
       */
        ...
        this.menuRenderer.renderMenuBackground(buttonPos, buttonMargin, widthTiles, heightTiles);
        this.fontRenderer.renderText(buttonMargin + 0.5f, buttonPos + 0.5f, "MUTLIPLAYER", widthTiles, heightTiles);
